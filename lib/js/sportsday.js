...
(function() {
  ...
  //  (1) Load the events table (once)
  //  (2) Load the list of events (once)
  //  (3) Check if we need to enable the refresh timer
  //  (4) Check if we need to show the survey alert
  sd.controller("global", function($rootScope, $location, $scope, $http) {
    ...
    // get forms
    $http.get(sdBuildQuery("summary!A3:B35", "ROWS", false))
      .then(function(res) {
        // success
    ...
    $scope.$on("$routeChangeStart", function($event, next) {
      if (next.controller === "events" || next.controller === "survey" || next.controller === "about" || next.controller === "404" || next.controller === "timetable") {
        ...
        if (getCookie("stopSurveyAlert") === "false") {
          swal({
            ...
            content: {
              element: "a",
              attributes: {
                innerHTML: "Did you know that we are testing a new version of the website for 2020? Click here try it out.",
                href: "https://mgssportsday.netlify.com/",
                target: "_blank"
              }
            }
          })
  ...
  sd.controller("timetable", function() {
    // Empty controller, but has to exist for timer disable.
  });
  ...
  sd.controller("event", function($scope, $http, $routeParams, $location, $rootScope) {
    ...
    if (_.isEmpty(matchingEvent)) {
      ...
      // === RESULTS ===
      var ssRange = "y" + $scope.year + "_results!" + intToSpreadsheetColLetter($scope.eventDetails.startingCol) + "6:" +  intToSpreadsheetColLetter($scope.eventDetails.startingCol + 4) + "24";
      ...
      $http.get(sdBuildQuery("y" + $scope.year + "_records!A4:J15", "ROWS", false))
        .then(function(res) {
          ...
          } else if ($scope.record.doScore === 1) {
            $scope.record.remarks = "✨ This year, " + $scope.record.currentHolder + " equalled the standing record! 1 bonus point awarded to " + $scope.year + $scope.record.currentForm + ".";
          } else if ($scope.record.doScore === 2) {
            $scope.record.remarks = "🎉 This year, " + $scope.record.currentHolder + " beat the standing record! 2 bonus points awarded to " + $scope.year + $scope.record.currentForm + ".";
          }
        }, function(res) {
          // failure
          apiErr(res);
      ...
      $http.get(sdBuildQuery("y" + $scope.year + "_records!A4:J15", "ROWS", false))
        .then(function(res) {
          ...
          for (var i = 0; i < $scope.details.length; i++) {
            ...
            // set the remark
            if ($scope.details[i].doScore === 2) {
              $scope.details[i].remark = "🎉 New record!";
            } else if ($scope.details[i].doScore === 1) {
              $scope.details[i].remark = "✨ Record equalled!";
            } else {
