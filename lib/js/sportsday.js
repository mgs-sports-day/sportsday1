...
(function() {
  ...
  // Runs on all views to check:
  //  (1) If we need to enable the refresh timer
  //  (2) If we need to show the survey alert
  ...
  sd.controller("event", function($scope, $http, $routeParams, $location) {
    // set event name based on route request
    var requested = $routeParams.eventID;
    $http.get(sdBuildQuery("event_list!A2:F13", "ROWS", false))
      .then(function(res) {
        // success
        res = sdParseRes(res.data);
        var matching = _.where(res, {db: requested});
