...
(function() {
  ...
  sd.controller("office", function($scope, $rootScope, $location) {
    ...
    $scope.stage3csv = function(type) {
      if (type === "Track") {
        var csvContent = Papa.unparse(generatedTrack);
        var csvName = "reporting_track_data.csv";
      }
      if (type === "Field") {
        var csvContent = Papa.unparse(generatedField);
        var csvName = "reporting_field_data.csv";
      }
