      ...
      $http.get(sdBuildQuery("y" + $scope.year + "_records!A4:J15", "ROWS", false))
        .then(function(res) {
          ...
          if ($scope.record.doScore === 0) {
            ...
            $scope.record.remarks = "✨ This year, " + $scope.record.currentHolder + " equalled the standing record! " + $rootScope.recordBonusValues.equal + " bonus point(s) awarded to " + $scope.year + $scope.record.currentForm + ".";
          } else if ($scope.record.doScore === $rootScope.recordBonusValues.beat) {
            $scope.record.remarks = "🎉 This year, " + $scope.record.currentHolder + " beat the standing record! " + $rootScope.recordBonusValues.beat + " bonus point(s) awarded to " + $scope.year + $scope.record.currentForm + ".";
          }
        }, function(res) {
          // failure
          apiErr(res);
