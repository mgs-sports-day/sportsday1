...
window.onload = function() {
    outdatedBrowser({
        bgColor: "#007bff",
        color: "#ffffff",
        lowerThan: "font-kerning",
        languagePath: "https://cdn.jsdelivr.net/gh/burocratik/outdated-browser@1.1.5/outdatedbrowser/lang/en.html"
    });
};

(function() {
    "use strict";
    let sd = angular.module('sdApp', ['ngRoute', 'ngAnimate', 'ngCookies', 'googlechart', 'angular-google-analytics', 'angular-loading-bar']);
    ...
    sd.controller("global", function($rootScope, $location, $scope, $cookies) {
        const d = new Date(); // create new blank data
        d.setHours(d.getHours() + 6) // set date to +6 hours
        const cookieOptions = { expires: d }; // set cookie options to expire in 6 hours
        $cookies.put("stopSurveyAlert", "false", cookieOptions); // use angularjs $cookies function to create stopSurveyAlert cookie

        $rootScope.pagesCount = 0; // set initially to 0
        $scope.$on("$routeChangeStart", function($event, next) {
            $rootScope.timerEnabled = !(next.controller === "survey" || next.controller === "about" || next.controller === "404" || next.controller === "insight");
            $rootScope.pagesCount += 1;
            if ($rootScope.pagesCount >= 11) { // one less than 10, as first page load counts for 2
                if ($cookies.get("stopSurveyAlert") === "false") {
                    swal({
                        ...
                        .then(function(goAhead){
                            ...
                            $cookies.put('stopSurveyAlert', "true", cookieOptions); // set the cookie to true no matter the outcome
                        });
                }
            }
        });
    ...
    sd.controller('activity', function($scope, $http, $routeParams) {
        let activityName = $routeParams.activityID;
        $scope.activityName = sdPrettifyName(activityName);
        let cr = c.t.d[activityName];
        let gets = [cr.a.pos, cr.a.pts, cr.a.name, cr.b.pos, cr.b.pts, cr.b.name, cr.c.pos, cr.c.pts, cr.c.name, cr.total];
        let query = "select #year#, #form#, " + gets.join(",") + "";
    ...
    sd.controller('form', function($scope, $http, $routeParams) {
        let formName = $routeParams.formID.replace('SLASH', '/');
        $scope.formName = formName;
        let query = "select * where #combined# = '" + formName + "'";
        $http.get(sdBuildQuery(query, c.baseURL))
            .then(function(res) {
                res = sdParseRes(res.data);
                $scope.form = res.table.rows;
                let formActivities = {};
                for (let i = 0; i < res.table.cols.length; i++) {
                    if (/(.*)(_)([^_]*)(_)(.*)/.test(res.table.cols[i].label)) {
                        let col = res.table.cols[i].label;
                        col = col.split("_");
                        let colCat = col[0];
    ...
    sd.controller("insight", function($scope, $routeParams, $http) {
        let insightID = parseInt($routeParams.insightID);
        let chartAreaOptions = {
            left: "5px",
            top: "10%",
            width: "100%",
            height: "500px"
        ...
        switch (insightID) {
            case 1:
                ...
                $http.get(sdBuildQuery("select #year#,#form#,#total#", c.baseURL))
                    .then(function(res) {
                        ...
                        for (let b = 0; b < Object.keys(filter).length; b++) {
                            if (yearGroupTotals[b] === undefined || yearGroupTotals[b] === null) {
                                yearGroupTotals.push(0);
                            }
                            for (let x = 0; x < filter[Object.keys(filter)[b]].length; x++) {
            ...
            case 3:
                ...
                $scope.update = function(e) {
                    $scope.chartLoading = true;
                    let sport = e.insight3Sport;
                    let col = c.t.d[sport].total;
